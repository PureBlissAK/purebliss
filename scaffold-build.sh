...
detect_current_stage() {
    ...
    if [[ -d "/opt/dev-purebliss/.github/workflows" ]] || [[ -f "/opt/dev-purebliss/start-all-services.sh" ]]; then
        detected_stage=5
    fi
...
assess_current_infrastructure() {
    ...
    [[ -f "/opt/dev-purebliss/start-all-services.sh" ]] && echo "✓ Service startup script present" || echo "✗ Service startup script missing"
    [[ -d "/opt/dev-purebliss/.github/workflows" ]] && echo "✓ GitHub workflows present" || echo "✗ GitHub workflows missing"
    [[ -f "/opt/dev-purebliss/setup-vault.sh" ]] && echo "✓ Vault setup script present" || echo "✗ Vault setup script missing"
    echo ""
...
validate_stage_5() {
    ...
    if [[ ! -f "/opt/dev-purebliss/start-all-services.sh" ]]; then
...
apply_security_foundation_config() {
    ...
    if [[ -f "/opt/dev-purebliss/setup-vault.sh" ]]; then
        log_message "INFO" "Enhancing Vault configuration"
        bash /opt/dev-purebliss/setup-vault.sh
    fi
...
apply_automation_cicd_config() {
    ...
    if [[ -f "/opt/dev-purebliss/start-purebliss-orchestrator.sh" ]]; then
        log_message "INFO" "Enhancing orchestrator configuration"
        # Run orchestrator in enhancement mode (preserve existing state)
        bash /opt/dev-purebliss/start-purebliss-orchestrator.sh
    fi
...
apply_elite_compliance_config() {
    ...
    if [[ -f "/opt/dev-purebliss/start-all-services.sh" ]]; then
        ...
        bash /opt/dev-purebliss/start-all-services.sh
    fi
}
