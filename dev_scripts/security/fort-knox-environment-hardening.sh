...
# ═══════════════════════════════════════════════════════════════════════════════════
# FORT_KNOX_ENVIRONMENT_HARDENING_SH
# ═══════════════════════════════════════════════════════════════════════════════════
...
SCRIPT_NAME="fort-knox-environment-hardening.sh"
...
fort_knox_environment_hardening_log_info() {
    local message="$1"
...
fort_knox_environment_hardening_log_error() {
    local message="$1"
...
fort_knox_environment_hardening_log_success() {
    local message="$1"
...
fort_knox_environment_hardening_auto_commit_wrapper() {
    local commit_message="${1:-"Auto-commit: ${SCRIPT_NAME} executed successfully"}"
    local validation_command="${2:-}"
    
    fort_knox_environment_hardening_log_info "Starting auto-commit wrapper for successful execution"
    
    # Run validation if provided
    if [[ -n "$validation_command" ]]; then
        fort_knox_environment_hardening_log_info "Running validation: $validation_command"
        if eval "$validation_command"; then
            fort_knox_environment_hardening_log_success "Validation passed - proceeding with auto-commit"
        else
            fort_knox_environment_hardening_log_error "Validation failed - skipping auto-commit"
            return 1
        fi
    fi
    ...
    if [[ -f "/opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh" ]]; then
        fort_knox_environment_hardening_log_info "Using Pure Bliss Elite auto-commit system"
        /opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh             "${SCRIPT_CATEGORY}" "$commit_message" "${SCRIPT_NAME}"
    elif command -v auto_commit_push_wrapper >/dev/null 2>&1; then
        auto_commit_push_wrapper "${SCRIPT_NAME}" "$commit_message"
    else
        fort_knox_environment_hardening_log_info "Auto-commit system not available - manual commit required"
        fort_knox_environment_hardening_log_info "Recommended commit message: $commit_message"
        fort_knox_environment_hardening_log_info "See: /opt/dev-purebliss/dev_scripts/automation/AUTO_COMMIT_SYSTEM_GUIDE.md"
    fi
}

# Wrapper for comprehensive script completion with auto-commit
fort_knox_environment_hardening_complete_with_commit() {
    local final_message="${1:-"${SCRIPT_NAME} completed successfully"}"
    local validation_command="${2:-}"
    
    # Log successful completion
    fort_knox_environment_hardening_log_success "$final_message"
    
    # Execute auto-commit wrapper
    fort_knox_environment_hardening_auto_commit_wrapper "Auto-commit: $final_message" "$validation_command"
    
    # Final status
    fort_knox_environment_hardening_log_success "${SCRIPT_NAME} execution and auto-commit completed"
}
