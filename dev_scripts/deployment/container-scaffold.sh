...
analyze_existing_container() {
    ...
    if [[ -n "$dockerfile_found" ]]; then
        ...
        echo "• Use: /opt/dev-purebliss/dev_scripts/core/container-scaffold.sh build $service 3"
    else
        ...
        echo "• Use: /opt/dev-purebliss/dev_scripts/core/container-scaffold.sh generate $service"
    fi
...
analyze_all_containers() {
    ...
    echo "/opt/dev-purebliss/dev_scripts/core/container-scaffold.sh analyze nginx    # Detailed analysis"
    echo "/opt/dev-purebliss/dev_scripts/core/container-scaffold.sh generate nginx   # Generate enhanced Dockerfile"
    echo "/opt/dev-purebliss/dev_scripts/core/container-scaffold.sh build nginx 3    # Build to phase 3"
    echo "/opt/dev-purebliss/dev_scripts/core/container-scaffold.sh validate nginx 3 # Validate phase 3"
}
...
main() {
    ...
    case $command in
        ...
        "help")
            ...
            echo ""
            echo "Examples:"
            echo "  $0 analyze                 Analyze all existing containers"
            echo "  $0 analyze nginx           Analyze existing nginx container"
            echo "  $0 build nginx 3           Build nginx up to phase 3"
            echo "  $0 generate redis          Generate Redis Dockerfile"
            echo "  $0 validate postgres 2     Validate postgres phase 2"
            echo ""
            echo "Integration with existing work:"
            echo "  - Uses existing Dockerfiles from /opt/dev-purebliss/services/"
            echo "  - Preserves Vault integration and entrypoint scripts"
            echo "  - Enhances with progressive phases for error reduction"
            ;;
        *)
