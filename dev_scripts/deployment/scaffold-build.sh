...
scaffold_build_auto_commit_wrapper() {
    local commit_message="${1:-"Auto-commit: ${SCRIPT_NAME} executed successfully"}"
    local validation_command="${2:-}"
    
    scaffold_build_log_info "Starting auto-commit wrapper for successful execution"
    
    # Run validation if provided
    if [[ -n "$validation_command" ]]; then
        scaffold_build_log_info "Running validation: $validation_command"
        if eval "$validation_command"; then
            scaffold_build_log_success "Validation passed - proceeding with auto-commit"
        else
            scaffold_build_log_error "Validation failed - skipping auto-commit"
            return 1
        fi
    fi
    ...
    if [[ -f "/opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh" ]]; then
        scaffold_build_log_info "Using Pure Bliss Elite auto-commit system"
        /opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh             "${SCRIPT_CATEGORY}" "$commit_message" "${SCRIPT_NAME}"
    elif command -v auto_commit_push_wrapper >/dev/null 2>&1; then
        auto_commit_push_wrapper "${SCRIPT_NAME}" "$commit_message"
    else
        scaffold_build_log_info "Auto-commit system not available - manual commit required"
        scaffold_build_log_info "Recommended commit message: $commit_message"
        scaffold_build_log_info "See: /opt/dev-purebliss/dev_scripts/automation/AUTO_COMMIT_SYSTEM_GUIDE.md"
    fi
}

# Wrapper for comprehensive script completion with auto-commit
scaffold_build_complete_with_commit() {
    local final_message="${1:-"${SCRIPT_NAME} completed successfully"}"
    local validation_command="${2:-}"
    
    # Log successful completion
    scaffold_build_log_success "$final_message"
    
    # Execute auto-commit wrapper
    scaffold_build_auto_commit_wrapper "Auto-commit: $final_message" "$validation_command"
    
    # Final status
    scaffold_build_log_success "${SCRIPT_NAME} execution and auto-commit completed"
}
...
# Purpose: Pure Bliss automation script
# Category: scaffolding
# Dependencies: docker,curl,vault,postgresql-client
# Usage: /opt/dev-purebliss/dev_scripts/core/scaffold-build.sh [options]
# Exit Codes: 0=success, 1=error, 2=warning
# Log Output: /opt/my-secure-ha-stack/logs/dev-environment-setup.log
# Health Validation: optional
# Vault Required: conditional
