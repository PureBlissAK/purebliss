...
SCRIPT_NAME="comprehensive-health-check.sh"
...
comprehensive_health_check_log_info() {
    local message="$1"
...
comprehensive_health_check_log_error() {
    local message="$1"
...
comprehensive_health_check_log_success() {
    local message="$1"
...
comprehensive_health_check_auto_commit_wrapper() {
    local commit_message="${1:-"Auto-commit: ${SCRIPT_NAME} executed successfully"}"
    local validation_command="${2:-}"
    
    comprehensive_health_check_log_info "Starting auto-commit wrapper for successful execution"
    
    # Run validation if provided
    if [[ -n "$validation_command" ]]; then
        comprehensive_health_check_log_info "Running validation: $validation_command"
        if eval "$validation_command"; then
            comprehensive_health_check_log_success "Validation passed - proceeding with auto-commit"
        else
            comprehensive_health_check_log_error "Validation failed - skipping auto-commit"
            return 1
        fi
    fi
    ...
    if [[ -f "/opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh" ]]; then
        comprehensive_health_check_log_info "Using Pure Bliss Elite auto-commit system"
        /opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh             "${SCRIPT_CATEGORY}" "$commit_message" "${SCRIPT_NAME}"
    elif command -v auto_commit_push_wrapper >/dev/null 2>&1; then
        auto_commit_push_wrapper "${SCRIPT_NAME}" "$commit_message"
    else
        comprehensive_health_check_log_info "Auto-commit system not available - manual commit required"
        comprehensive_health_check_log_info "Recommended commit message: $commit_message"
        comprehensive_health_check_log_info "See: /opt/dev-purebliss/dev_scripts/automation/AUTO_COMMIT_SYSTEM_GUIDE.md"
    fi
}

# Wrapper for comprehensive script completion with auto-commit
comprehensive_health_check_complete_with_commit() {
    local final_message="${1:-"${SCRIPT_NAME} completed successfully"}"
    local validation_command="${2:-}"
    
    # Log successful completion
    comprehensive_health_check_log_success "$final_message"
    
    # Execute auto-commit wrapper
    comprehensive_health_check_auto_commit_wrapper "Auto-commit: $final_message" "$validation_command"
    
    # Final status
    comprehensive_health_check_log_success "${SCRIPT_NAME} execution and auto-commit completed"
}
...
function test_orchestrator_integration() {
    ...
    local orchestrator_script="/opt/dev-purebliss/dev_scripts/automation/start-all-services.sh"
...
function main() {
    ...
    echo "=== grafana Vault Integration ==="    test_grafana_vault_integration    echo ""
    ...
    if [[ "$error_count" -eq 0 && "$warning_count" -eq 0 ]]; then
        ...
        echo "   Restart with orchestrator: /opt/dev-purebliss/dev_scripts/automation/start-all-services.sh"
...
function test_redis_vault_integration() {
    log_check "Testing redis Vault integration..."
    # Container health check
    if test_container_health "purebliss-redis"; then
        log_success "redis container healthy"
    else
        log_error "redis container not healthy"
        return 1
    fi
    # Vault integration validation
    if [[ -x "/opt/dev-purebliss/services/redis/validate-redis-vault-integration.sh" ]]; then
        if /opt/dev-purebliss/services/redis/validate-redis-vault-integration.sh; then
            log_success "redis Vault integration validated"
        else
            log_error "redis Vault integration validation failed"
            return 1
        fi
    else
        log_warning "redis validation script not found"
    fi
}

function test_nginx_vault_integration() {
    log_check "Testing nginx Vault integration..."
    # Container health check
    if test_container_health "purebliss-nginx"; then
        log_success "nginx container healthy"
    else
        log_error "nginx container not healthy"
        return 1
    fi
    # Vault integration validation
    if [[ -x "/opt/dev-purebliss/services/nginx/validate-nginx-vault-integration.sh" ]]; then
        if /opt/dev-purebliss/services/nginx/validate-nginx-vault-integration.sh; then
            log_success "nginx Vault integration validated"
        else
            log_error "nginx Vault integration validation failed"
            return 1
        fi
    else
...
function test_grafana_vault_integration() {
    log_check "Testing grafana Vault integration..."
    # Container health check
    if test_container_health "purebliss-grafana"; then
        log_success "grafana container healthy"
    else
        log_error "grafana container not healthy"
        return 1
    fi
    # Vault integration validation
    if [[ -x "/opt/dev-purebliss/services/grafana/validate-grafana-vault-integration.sh" ]]; then
        if /opt/dev-purebliss/services/grafana/validate-grafana-vault-integration.sh; then
            log_success "grafana Vault integration validated"
        else
            log_error "grafana Vault integration validation failed"
            return 1
        fi
    else
