...
SCRIPT_NAME="validate-container-health.sh"
...
validate_container_health_log_info() {
    local message="$1"
...
validate_container_health_log_error() {
    local message="$1"
...
validate_container_health_log_success() {
    local message="$1"
...
validate_container_health_auto_commit_wrapper() {
    local commit_message="${1:-"Auto-commit: ${SCRIPT_NAME} executed successfully"}"
    local validation_command="${2:-}"
    
    validate_container_health_log_info "Starting auto-commit wrapper for successful execution"
    
    # Run validation if provided
    if [[ -n "$validation_command" ]]; then
        validate_container_health_log_info "Running validation: $validation_command"
        if eval "$validation_command"; then
            validate_container_health_log_success "Validation passed - proceeding with auto-commit"
        else
            validate_container_health_log_error "Validation failed - skipping auto-commit"
            return 1
        fi
    fi
    ...
    if [[ -f "/opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh" ]]; then
        validate_container_health_log_info "Using Pure Bliss Elite auto-commit system"
        /opt/dev-purebliss/dev_scripts/automation/auto-commit-trigger.sh             "${SCRIPT_CATEGORY}" "$commit_message" "${SCRIPT_NAME}"
    elif command -v auto_commit_push_wrapper >/dev/null 2>&1; then
        auto_commit_push_wrapper "${SCRIPT_NAME}" "$commit_message"
    else
        validate_container_health_log_info "Auto-commit system not available - manual commit required"
        validate_container_health_log_info "Recommended commit message: $commit_message"
        validate_container_health_log_info "See: /opt/dev-purebliss/dev_scripts/automation/AUTO_COMMIT_SYSTEM_GUIDE.md"
    fi
}

# Wrapper for comprehensive script completion with auto-commit
validate_container_health_complete_with_commit() {
    local final_message="${1:-"${SCRIPT_NAME} completed successfully"}"
    local validation_command="${2:-}"
    
    # Log successful completion
    validate_container_health_log_success "$final_message"
    
    # Execute auto-commit wrapper
    validate_container_health_auto_commit_wrapper "Auto-commit: $final_message" "$validation_command"
    
    # Final status
    validate_container_health_log_success "${SCRIPT_NAME} execution and auto-commit completed"
}
...
# --- PURE BLISS CONTAINER METADATA VALIDATION ---
validate_container_metadata() {
    ...
    case "$service_name" in
        ...
        vault)
            # Vault uses the official image, no custom Dockerfile
            log_info "Vault uses official hashicorp/vault image - no custom Dockerfile validation needed"
            return 0;;
        nginx|loki|prometheus|grafana|letsencrypt)
            # These services may use official images or custom builds
            log_info "$service_name may use official image - metadata validation skipped"
            return 0;;
        *)
    ...
    if [ -f "$dockerfile_path" ]; then
        if ! grep -q "PURE BLISS CONTAINER METADATA" "$dockerfile_path"; then
            log_error "$service_name Dockerfile missing PURE BLISS CONTAINER METADATA comment at $dockerfile_path"
            return 1
        else
            # Extract container purpose and scaffolding phase for validation logging
            ...
            log_success "$service_name Dockerfile contains PURE BLISS CONTAINER METADATA"
            [ -n "$purpose" ] && log_info "Container Purpose: $purpose"
            [ -n "$phase" ] && log_info "Scaffolding Phase: $phase"
        fi
    else
        log_warn "$service_name Dockerfile not found at expected path: $dockerfile_path"
        return 0
    fi
...
perform_deep_health_troubleshooting() {
    ...
    case "$issue_type" in
        ...
        "health_check_failed")
            ...
            log_health "INFO" "COMMAND: docker exec $CONTAINER_NAME curl -fk http://localhost:8080/health || true"
            ;;
        "dependency_failure")
...
validate_nginx_endpoints() {
    ...
    if curl -fk -s -m 10 "http://localhost/health" -H "Host: dev.purebliss.app" >/dev/null 2>&1; then
    ...
    if curl -fk -s -k -m 10 "https://localhost/health" -H "Host: dev.purebliss.app" >/dev/null 2>&1; then
    ...
    if curl -fk -s -m 10 "http://localhost/status" -H "Host: dev.purebliss.app" >/dev/null 2>&1; then
...
validate_vault_endpoints() {
    ...
    if docker exec "$CONTAINER_NAME" curl -fk -s -m 10 "https://localhost:8200/v1/sys/health" >/dev/null 2>&1; then
...
validate_plane_endpoints() {
    ...
    if docker exec "$CONTAINER_NAME" curl -fk -s -m 10 "http://localhost:3000/api/health" >/dev/null 2>&1; then
...
validate_codeserver_endpoints() {
    ...
    if docker exec "$CONTAINER_NAME" curl -fk -s -m 10 "http://localhost:8080/healthz" >/dev/null 2>&1; then
