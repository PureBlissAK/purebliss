...
FALLBACK_LOG="/tmp/vault-entrypoint.log"

# Robust log function: fallback if log dir missing
log_dest() {
...
# Logging functions
function log_info() {
    echo "[$(date)] [Vault Entrypoint] INFO: $1" | tee -a "$(log_dest)"
}

# Set default VAULT_ADDR to HTTPS if not set
...
function log_error() {
    echo "[$(date)] [Vault Entrypoint] ERROR: $1" | tee -a "$(log_dest)"
}

function log_success() {
    echo "[$(date)] [Vault Entrypoint] SUCCESS: $1" | tee -a "$(log_dest)"
}
