function vault_letsencrypt_vault_integration_fix() {
    ...
    # Re-run onboarding
    if /opt/dev-purebliss/dev_scripts/automation/start-all-services.sh letsencrypt >> "$LOG_FILE" 2>&1; then
...
function vault_nginx_pki_integration_fix() {
    ...
    if /opt/dev-purebliss/dev_scripts/automation/start-all-services.sh nginx >> "$LOG_FILE" 2>&1; then
...
function vault_redis_integration_fix() {
    ...
    if [[ -f "/opt/my-secure-ha-stack/secrets/vault_token" ]]; then
        ...
        if vault read redis/config/redis >/dev/null 2>&1; then
            ...
            /opt/dev-purebliss/dev_scripts/automation/start-all-services.sh redis
        fi
    else
