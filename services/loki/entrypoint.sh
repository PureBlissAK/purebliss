...
# Function to fetch dynamic secrets
fetch_dynamic_secrets() {
    if [[ -n "$VAULT_TOKEN" ]]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] Fetching dynamic secrets for Loki storage..." | tee -a "$LOG_FILE"
        
        # Test dynamic secret issuance (example for future storage integration)
        STORAGE_CREDS=$(curl -sk -H "X-Vault-Token: $VAULT_TOKEN" \
            "$VAULT_ENDPOINT/v1/loki-secrets/storage" 2>/dev/null)
        
        if [[ "$?" == "0" ]]; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] ✅ Dynamic secrets access validated" | tee -a "$LOG_FILE"
            return 0
        else
...
# 6. Start Loki with enhanced entrypoint
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting Loki service..." | tee -a "$LOG_FILE"

# Start Loki in background to allow upstream notification
