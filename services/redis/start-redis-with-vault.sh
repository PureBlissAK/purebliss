...
# redis Vault Integration Startup Script
# Configures Vault integration and starts redis with dynamic secrets
...
function log_action() {
    echo "[$(date)] REDIS_VAULT_START: $1" | tee -a "$LOG_FILE"
    echo "🔧 $1"
}

function log_success() {
    echo "[$(date)] REDIS_VAULT_START: ✅ SUCCESS: $1" | tee -a "$LOG_FILE"
    echo "✅ $1"
}

function log_error() {
    echo "[$(date)] REDIS_VAULT_START: ❌ ERROR: $1" | tee -a "$LOG_FILE"
    echo "❌ $1"
}
