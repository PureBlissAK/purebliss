...
services:
  $service:
    ...
    networks:
      - purebliss-net
    healthcheck:
      test: ["CMD-SHELL", "curl -f http://localhost:\${${service^^}_PORT:-8080}/health || exit 1"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 60s
...
services:
  $service:
    ...
    healthcheck:
      test: ["CMD-SHELL", "curl -fk https://localhost:\${${service^^}_HTTPS_PORT:-8443}/health || exit 1"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 60s
...
services:
  $service:
    ...
    healthcheck:
      test: ["CMD-SHELL", "curl -f http://localhost:\${${service^^}_PORT:-9090}/-/healthy || exit 1"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 60s
...
### Health Check
\`\`\`bash
/opt/dev-purebliss/comprehensive-health-check.sh | grep -A 10 "$service"
\`\`\`
