...
complete_postgres_validation() {
    ...
    for phase in "${validation_phases[@]}"; do
        ...
        if ! /opt/dev-purebliss/validate-container-health.sh postgres "validation-$phase" 2>/dev/null; then
...
complete_redis_validation() {
    ...
    for phase in "${validation_phases[@]}"; do
        ...
        if ! /opt/dev-purebliss/validate-container-health.sh redis "validation-$phase" 2>/dev/null; then
